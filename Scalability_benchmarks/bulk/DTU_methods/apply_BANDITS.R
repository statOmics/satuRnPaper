  ...
  timing <- system.time({
    ...
    if(dim(quantsf_counts)[1] > j){
        ...
        design$group <- sample(rep(c("A","B"),each=i))
      	...
      	equiv_classes_files <- equiv_classes_files[which(substring(equiv_classes_files,46,55)%in%colnames(quantsf_counts))]
	
	      message(equiv_classes_files[!file.exists(equiv_classes_files)])

	      eff_len <- readRDS("./Data/Scalability_data/eff_len.Rds")
