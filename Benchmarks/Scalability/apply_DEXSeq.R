...
run_DEXSeq <- function(L,countData,tx2gene) {
  message("DEXSeq")
  ...
  timing <- system.time({
	...
	sampleData <- design
        ...
        dxd <- DEXSeqDataSet(countData = quantsf_counts,
                             sampleData = sampleData,
                             design = ~ sample + exon + group:exon,
                             featureID = rownames(quantsf_counts),
        ...
        finalResult <- nrow(localRes[which(localRes$padj < 0.05),])
  ...
  output_DEXSeq<-  list(session_info = session_info,
                                 info = list(size = i, 
  ...
  assign("result", output_DEXSeq, envir=globalenv())
  rm(list = ls())
  gc()
}
