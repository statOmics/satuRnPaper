...
run_limmaDiffsplice <- function(L,countData,tx2gene) {
  message("limma_diffsplice")
  ...
  timing <- system.time({
	...
	if(length(which(colSums(quantsf_counts) == 0)) > 0){
        ...
        y <- DGEList(counts = quantsf_counts, group = design$group, genes = geneForEachTx)
        design <- model.matrix(~ design$group)

        v <- voom(y, design, plot=F)
        fit <- lmFit(v, design)
      
        ex <- limma::diffSplice(fit, geneid = "genes",verbose = F)
        limmaRes <- topSplice(ex, coef=2, test="t", number = Inf)
      
        finalResult <- nrow(limmaRes[which(limmaRes$FDR < 0.05),])
