...
if(TRUE) {
    ...
    #load(file='01_gtex_benchmark_data_filter.Rdata')
    #load(file='01_gtex_benchmark_data_2017_filter_new.Rdata')
    #load(file='01_gtex_benchmark_data_2017_filter_new_10.Rdata')
    load(file='01_gtex_benchmark_data_2017_filterevenmore.Rdata')
...
```{r}
if(TRUE) {
    #load(file='02_Gtex_DTU_analysis_filter_2017_full.Rdata')
    #load(file='02_Gtex_DTU_analysis_2017_filter_new_full.Rdata')
    #load(file='02_Gtex_DTU_analysis_2017_filter_new_10.Rdata')
    load(file='02_Gtex_DTU_analysis_2017_filterevenmore.Rdata')
...
visualize_datasets <- function(dataset,metaInfo,nFilters,nSampleSizes,methods,colors,titles,selection) {
    ...
    for (j in 1:nPlots) {
        ...
        # gtexBenchmark_current_outer <- gtexBenchmark_current_outer[unique(grep(paste(methods,collapse="|"), 
        #                 names(gtexBenchmark_current_outer), value=TRUE))] # select the requested methods 
        # !!! the above line gets the results list in the same order as the original list, and not in the order of the requested methods!!!
        # Therefore, I alternatively do  this.
        ...
        gtexBenchmark_current_outer <- gtexBenchmark_current_outer[selected_methods]
        ...
        # rownames(resList[[4]][[1]]) <- paste0(rownames(resList[[4]][[1]]), "_4")
        # rownames(resList[[5]][[1]]) <- paste0(rownames(resList[[5]][[1]]), "_5")
        # rownames(resList[[6]][[1]]) <- paste0(rownames(resList[[6]][[1]]), "_6")
        # rownames(resList[[7]][[1]]) <- paste0(rownames(resList[[7]][[1]]), "_7")
        # rownames(resList[[8]][[1]]) <- paste0(rownames(resList[[8]][[1]]), "_8")
        # rownames(resList[[9]][[1]]) <- paste0(rownames(resList[[9]][[1]]), "_9")
        # rownames(resList[[10]][[1]]) <- paste0(rownames(resList[[10]][[1]]), "_10")
        # pvalues_full <- rbind(resList[[1]][[1]],resList[[2]][[1]],resList[[3]][[1]],resList[[4]][[1]],resList[[5]][[1]],resList[[6]][[1]],resList[[7]][[1]],resList[[8]][[1]],resList[[9]][[1]],resList[[10]][[1]])
        ...
        # rownames(resList[[4]][[2]]) <- paste0(rownames(resList[[4]][[2]]), "_4")
        # rownames(resList[[5]][[2]]) <- paste0(rownames(resList[[5]][[2]]), "_5")
        # rownames(resList[[6]][[2]]) <- paste0(rownames(resList[[6]][[2]]), "_6")
        # rownames(resList[[7]][[2]]) <- paste0(rownames(resList[[7]][[2]]), "_7")
        # rownames(resList[[8]][[2]]) <- paste0(rownames(resList[[8]][[2]]), "_8")
        # rownames(resList[[9]][[2]]) <- paste0(rownames(resList[[9]][[2]]), "_9")
        # rownames(resList[[10]][[2]]) <- paste0(rownames(resList[[10]][[2]]), "_10")
        # truth_file_full <- rbind(resList[[1]][[2]],resList[[2]][[2]],resList[[3]][[2]],resList[[4]][[2]],resList[[5]][[2]],resList[[6]][[2]],resList[[7]][[2]],resList[[8]][[2]],resList[[9]][[2]],resList[[10]][[2]])
        ...
        # save plot
        png(paste("/Users/jg/Desktop/PhD/DTU_project/Github/dtuPaper/Results/Gtex_benchmark/plot_icobra_2017_filterevenmore_", selection[j], "_", ".png", sep = ""),
        width     = 5.5,
        height    = 4.5,
        units     = "in",
        res       = 200,
...
```{r}
selection <- c("5_rep_.(_|0_)filterLenient","5_rep_.(_|0_)filterStringent")

colors<- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "black")
names(colors) <- c("DEXSeq","DRIMSeq","DoubleExpSeq","edgeR_diffsplice", "limma_diffsplice" ,"NBSplice","quasiBinomial")

#methods <- c("DEXSeq","DRIMSeq","DoubleExpSeq","edgeR_diffsplice", "limma_diffsplice" ,"NBSplice","quasiBinomial")

methods <- c("quasiBinomial","DoubleExpSeq","edgeR_diffsplice", "limma_diffsplice")
...
```{r}
selection <- c("20_rep_.(_|0_)filterLenient","50_rep_.(_|0_)filterLenient","20_rep_.(_|0_)filterStringent","50_rep_.(_|0_)filterStringent")
...
titles <- c("20 versus 20 - edgeR filter","50 versus 50 - edgeR filter","20 versus 20 - DRIMSeq filter","50 versus 50 - DRIMSeq filter")
...
```
selection <- c("50_rep_._filterLenient","90_rep_._filterLenient","50_rep_._filterStringent","90_rep_._filterStringent")
...
titles <- c("50 versus 50 - edgeR filter","90 versus 90 - edgeR filter","50 versus 50 - DRIMSeq filter","90 versus 90 - DRIMSeq filter")
