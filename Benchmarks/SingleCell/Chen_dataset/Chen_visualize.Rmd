...
if(TRUE) {
    ...
    #load(file='01_Chen_benchmark_data_scaledTPM.Rdata')
    
    ### Concatenate
    metaInfo <- c(
        ChenBenchmarkLenient,ChenBenchmarkStringent
    )
...
```{r}
if(TRUE) {
    load(file='02_Chen_DTU_analysis_full_count.Rdata')
    #load(file='02_Chen_DTU_analysis_full_scaledTPM.Rdata')
    ...
    rm(ChenDtuBenchmark_quasiBinomial,
        ChenDtuBenchmark_limmaDiffsplice,
        ChenDtuBenchmark_DEXSeq,
        ChenDtuBenchmark_DRIMSeq,
        ChenDtuBenchmark_edgeRdiffsplice,
...
visualize_datasets <- function(dataset,metaInfo,nFilters,nSampleSizes,nRep,nmethods,selection,selection2) {
    ...
    for (j in 1:nPlots) {
        ...
        if (nmethods == 6){
            ...
            plot <- plot_fdrtprcurve(cobraplot, xaxisrange = c(0, 0.4), yaxisrange = c(0.4,1))
        ...
        png(paste("/Users/jg/Desktop/PhD/DTU_project/Github/dtuPaper/Results/Chen_benchmark/plot_icobra_tpm_", selection2[j], "_", sub("^[^f]*", "", selection[j]), ".png", sep = ""),
            width     = 5.5,
            height    = 4.5,
            units     = "in",
            res       = 200,
