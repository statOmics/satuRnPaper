...
**In order to run this script (3_Chen_visualize.Rmd), the datasets 02_Chen_benchmark_datasets_count.Rdata and 03_Chen_DTU_results_count.Rdata are required.** These files can either be generated with the 1_Chen_prepare.Rmd and 2_Chen_DTU.Rmd scripts, respectively, or downloaded from Zenodo  and put in the data folder of this GitHub repository.
...
```{r}
load(file="./Data/03_Chen_DTU_results_count.Rdata")
...
visualize_datasets <- function(dataset,metaInfo,nFilters,nSampleSizes,nRep,nmethods,selection,selection2) {
    ...
    for (j in 1:nPlots) {
      ...
      png(paste("./Performance_benchmarks/SingleCell/Chen_dataset/Results/FDRTPR_", selection2[j], "_", sub("^[^f]*", "", selection[j]), "_count.png", sep = ""),
            width     = 5.5,
            height    = 4.5,
            units     = "in",
            res       = 200,
