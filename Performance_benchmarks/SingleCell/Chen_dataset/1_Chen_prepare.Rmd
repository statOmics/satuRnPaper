...
**In order to run this script (1_Chen_perpare.Rmd), the corresponding salmon quantification files Chen_counts.Rds should either be obtained by running 0_Chen_getData.Rmd or downloaded from Zenodo. In addition, the metaData Chen_metadata.csv should also be downloaded from Zenodo. Both files must then be copied into the Data folder of this Github page**.

1. Chen_counts.Rds
2. Chen_metadata.csv

**If one does not want to run this script, its output can also be downloaded from Zenodo: 02_Chen_benchmark_datasets_count.Rdata**
...
```{r}
save(ChenBenchmarkLenient, ChenBenchmarkStringent, file="./Data/02_Chen_benchmark_datasets_count.Rdata")
#save(ChenBenchmarkLenient, ChenBenchmarkStringent, file="./Data/02_Chen_benchmark_datasets_scaledTPM.Rdata")
```
